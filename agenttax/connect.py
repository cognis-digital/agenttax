"""Native cognis-connect emit for agenttax — forward findings to any platform.

Maps agenttax's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    agenttax ... --format json | agenttax-emit --to stix
    agenttax-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        if 'severity' in rec and rec['severity'] in ['high', 'medium', 'low', 'none']:
            out['severity'] = rec['severity'].upper()
        if 'type' in rec:
            out['type'] = rec['type'].title()
